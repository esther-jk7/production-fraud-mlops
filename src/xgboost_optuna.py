...
# XGBoost handles scaling internally but we scale for consistency
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train)
X_val_scaled = scaler.transform(X_val)
X_test_scaled = scaler.transform(X_test)

# Class imbalance ratio — XGBoost uses this to weight fraud cases
scale_pos_weight = (y_train == 0).sum() / (y_train == 1).sum()
print(f"scale_pos_weight: {scale_pos_weight:.1f}")

# ── Optuna objective function ─────────────────────────────────────────────────
# Optuna calls this function for each trial, passing different hyperparameters
# each time. We return val PR-AUC — Optuna maximizes this.
def objective(trial):
    params = {
...
print("Each trial trains an XGBoost model and logs to MLflow.\n")

with mlflow.start_run(run_name="xgboost-optuna-search"):
    ...
    ax.set_title('Confusion Matrix — XGBoost (Best)')
    for i in range(2):
        for j in range(2):
    ...
    print(f"\nBaseline was: 0.7179")
    print(f"Improvement:  {test_metrics['test_pr_auc'] - 0.7179:+.4f}")